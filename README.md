...
### Реферальная ссылка Тёмы

В Super Admin отображается персональная ссылка партнёра. Для текущего бота она имеет вид:

```text
https://t.me/<bot_username>?start=ref_tema
```

Первый переход закрепляет Telegram-пользователя за Тёмой. В суперадминке можно указать магазин, этап сделки, сумму продажи и комиссию. Повторный переход по другой партнёрской ссылке источник не перезаписывает.
